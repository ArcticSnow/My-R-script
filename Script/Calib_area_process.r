...
setwd("F:/Phd/Course/Spatial Stat/Project/final_product")
# Load Data and prepare for analysis
Calib_area <- read.table("F:/Phd/Course/Spatial Stat/Project/Calib_area.xyz", quote="\"")

#Grd_Freq_krig(Calib_area,Xlim=c(0,2),Ylim=(c(-12,-10)),dx=.1,dy=.1,my.krig.dxdy=.01)


print(c(min(Calib_area[,1]),max(Calib_area[,1]),min(Calib_area[,2]),max(Calib_area[,2])))
East.offset <- mean(Calib_area[,1])
North.offset <- mean(Calib_area[,2])


Calib_area[,1] <- Calib_area[,1]-East.offset
Calib_area[,2] <- Calib_area[,2]-North.offset
...
data <- matrix(c(Calib_area[,1],Calib_area[,2],Calib_area[,3]),ncol=3)
