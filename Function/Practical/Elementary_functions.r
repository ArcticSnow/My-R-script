...
LoadPointCloud <- function(Trans,file,header){
  ...
  if(my.data[1,1]>500){
    ...
    print(paste("Xmin =",min(my.data[,1])))
    print(paste("Xmax =",max(my.data[,1])))
    print(paste("Ymin =",min(my.data[,2])))
    print(paste("Ymax =",max(my.data[,2])))
...
Data.prepare <- function(my.xyz,translation,xlim,ylim,dx,dy,return.xyz){
  if(is.list(my.xyz)){
    xyz <- my.xyz$Data
    if(missing(translation)){
      translation <- my.xyz$Translation
    }}
  else{
    xyz <- my.xyz
    if(missing(translation)){
  ...
  if(Sys.info()['nodename']=="Simon-Filhols-MacBook-Pro.local")
{
  ...
  source(paste(R.git.dir(),'Function/Practical/Basic_statmap_ptcloud.r',sep="/"))
       
  my.dx <- dx
  my.dy <- dy
  stat <- Basic.statmap.ptcloud(xyz,dx=my.dx,dy=my.dy,Xlim=xlim,Ylim=ylim)
